  ...
  io.on('connection', (socket: TypedSocket) => {
    ...
    socket.on('buzz', (callback) => {
      try {
        ...
        console.log(`Player ${buzzerEvent.username} buzzed in room ${room.code}`);
    ...
    socket.on('update-score', (data: UpdateScorePayload) => {
      try {
        ...
        // Update score
        gameService.updateScore(room, playerId, validated.playerId, validated.points);

        const targetPlayer = playerService.getPlayer(room, validated.playerId);
        if (targetPlayer) {
          console.log(`Score updated for ${targetPlayer.username}: ${targetPlayer.score}`);
